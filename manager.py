...
def phases_permit_new_job(phases, sched_cfg, d):
    ...
    if len(phases) >= sched_cfg['tmpdir_max_jobs'] or len(phases) >= sched_cfg['tmpdir'][d]['max_jobs']:
        return False
...
def maybe_start_new_plot(dir_cfg, sched_cfg, plotting_cfg):
    ...
    if (youngest_job_age < global_stagger):
        ...
        eligible = [ (d, phases) for (d, phases) in tmp_to_all_phases
                if phases_permit_new_job(phases, sched_cfg, d) ]
        ...
        if not eligible:
            ...
            dir2ph = { k:v for k,v in dstdirs_to_youngest_phase(jobs).items()
                if plot_util.df_b(k) > (len(job.job_phases_for_dstdir(k, jobs)) + 1) * plot_util.get_k32_plotsize() + 10 * plot_util.GB }

            unused_dirs = [ d for d in dir_cfg['dst']
                if d not in dir2ph.keys() and plot_util.df_b(d) > (len(job.job_phases_for_dstdir(d, jobs)) + 1) * plot_util.get_k32_plotsize() + 10 * plot_util.GB ]
            dstdir = ''
            if unused_dirs: 
